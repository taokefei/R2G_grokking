...
class TransformerLM(torch.nn.Module):
    ...
    def run_greedy(
        ...
        n_steps = src.shape[1]     # [B, S]（token ids）

        src = self.pos_embed(self.input_embed(src), 0) # input_embed(src)：
        ...
        pred_words = torch.argmax(last_embeddings, -1)  # [B]
        next_tgt = torch.cat(
            [
                self.pos_embed(
        ...
        all_outputs = [last_embeddings.unsqueeze(1)]  # [B, 1, vocab]
        ...
        for i in range(max_len):
            ...
            output = self.output_map(output)      # [B, 1, V]
            all_outputs.append(output)
            out_token = torch.argmax(output[:, -1], -1)
            running &= out_token != self.encoder_eos50
            out_len[running] = i + 1
            next_tgt = torch.cat(
                [
                    self.pos_embed(
        ...
        return TransformerResult.create(torch.cat(all_outputs, 1), out_len+1)
