...
def working_dir():
    USER = os.environ.get("USER") or os.environ.get("USERNAME") or "user" #USER = os.environ["USER"]
...
def get_base_transformer_lm(args, in_vocab, model_name=None):
    model = create_lm(len(in_vocab), args.vec_dim, args.n_heads,
                      args.encoder_n_layers,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--decoder_n_layers", type=int, default=0)
    ...
    parser.add_argument("--traindata", type=int, default=10000)
...
#    wandb.run.save()
    ...
    print(f"Total time: {total_seconds / 3600:.2f} h")
